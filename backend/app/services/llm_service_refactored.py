...
class LLMService:
    ...
    async def generate_response(
        ...
        if ConversationContextManager.should_skip_analysis(user_message, skip_analysis):
            return await self._handle_fast_path(user_message, pipeline_start, emotion_data)

        # Run full analysis pipeline
        return await self._handle_full_analysis(
            user_message, conversation_history, user_id, pipeline_start, emotion_data
        )

    async def _handle_fast_path(self, user_message: str, pipeline_start: float, emotion_data: Optional[Dict[str, Any]] = None) -> str:
    ...
    def initialize(self) -> bool:
        ...
        try:
            ...
            if self.llm_provider.is_available():
                ...
                self._initialization_error = "Provider not available, skipping guardrails"
                print(f"⚠️  {self._initialization_error}")
